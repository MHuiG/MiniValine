  ...
  init(option) {
    ...
    _root.alert = {
      ...
      show(o) {
        ...
        let _cBtn = `<button class="vcancel vbtn">${(o && o.ctxt) ||
          _root.i18n["cancel"]}</button>`;
        let _oBtn = `<button class="vsure vbtn">${(o && o.otxt) ||
      ...
      cq.then(rets => {
        ...
        if (len) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            let _parent_vcard = insertComment(
              rets[i],
      ...
      if (level >= MAX_NEST_LEVEL) {
        _root.v.Query.doCloudQuery(
          ...
          function(data) {
            ...
            if (count > 0) {
              var _show_children_wrapper = _vchild.querySelector(
              ...
              var _show_children = _show_children_wrapper.querySelector(
                ".vshow-children"
              );
      ...
      _vcard.innerHTML = `<div class="vcomment-body">
        ...
        "nick"
                                        ...
                                        <div class="vmeta-info">
                                            ${
																							comment.get("link")
                                                ? `<a class="vname" href="${comment.get("link")}" target="_blank" rel="nofollow" > ${comment.get("nick")}</a>`
                                                : `<span class="vname">${comment.get("nick")}</span>`
                                            }
																						${
																							emailHash==admin_email_md5.toLowerCase().trim()
      ...
      if (!!s) {
        ...
        if (s["mail"] != "") {
          ...
          el.setAttribute(
            "src",
            GRAVATAR_BASE_URL +
              crypto(s["mail"].toLowerCase().trim()) +
              "?size=80&d=https%3a%2f%2fgravatar.loli.net%2favatar%2f9e63c80900d106cbbec5a9f4ea433a3e.jpg%3fsize%3d80"
          );
        }
      }
      ...
      comment
        ...
        .then(commentItem => {
          ...
          if (defaultComment["rid"] === "") {
            ...
            // get children vlist
            let _vlist = _root.el.querySelector(
