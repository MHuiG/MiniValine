...
const crypto = require("blueimp-md5");
const format = require("string-format");
const Utils = require("./utils/domUtils");

const AVSdkUrl =
  "https://cdn.jsdelivr.net/npm/leancloud-storage@3/dist/av-min.js";
const JQueryUrl = "https://cdn.jsdelivr.net/npm/jquery@3/dist/jquery.min.js";
const GRAVATAR_BASE_URL = "https://gravatar.loli.net/avatar/";
const DEFAULT_EMAIL_HASH = "9e63c80900d106cbbec5a9f4ea433a3e";
...
let disable_av_init = false;
let MAX_NEST_LEVEL = 3;
let PAGE_SIZE = 6;
let admin_email_md5 = "";
...
class Valine {
  ...
  init(option) {
    ...
    admin_email_md5 = option.admin_email_md5 || "";
    const lang = option.lang || "en";
    _root.i18n = option.i18n || i18n_set[lang];
    defaultComment.url =
      option.pathname || location.pathname.replace(/\/$/, "");
    try {
      const el =
        toString.call(option.el) === "[object HTMLDivElement]"
          ? option.el
      ...
      const eleHTML = `<div id="vinputs-placeholder">
                            <div class="vinputs-wrap">
                                <p class="vcancel-comment-reply" href="#" rel="nofollow" style="display:none">${
                                  _root.i18n.cancel_reply
                                }</p>
                                <div class="vinputs-area">
                                    <div class="textarea-wrapper">
                                        <div class="comment_trigger">
                                            <div class="avatar"><img class="visitor_avatar lazyload" data-src="${`${GRAVATAR_BASE_URL +
                                              DEFAULT_EMAIL_HASH}?size=80`}"></div>
      ...
      // Empty Data
      const vempty = _root.el.querySelector(".vempty");
      _root.nodata = {
        show(txt) {
      ...
      // load smiles image
      const _smile_wrapper = _root.el.querySelector(".vsmile-icons");
      const smile_names = option.emoticon_list || [];
      for (const i in smile_names) {
        const img = document.createElement("img");
        img.setAttribute(
          "data-src",
          `${option.emoticon_url}/${smile_names[i]}`
        );
    ...
    const vloading = _root.el.querySelector(".vloading");
    ...
    const vsubmitting = _root.el.querySelector(".vsubmitting");
    ...
    const _mark = _root.el.querySelector(".vmark");
    // alert
    _root.alert = {
      /**
       * {
      ...
      show(o) {
        ...
        const _vbtns = _mark.querySelector(".vbtns");
        const _cBtn = `<button class="vcancel vbtn">${(o && o.ctxt) ||
          _root.i18n.cancel}</button>`;
        const _oBtn = `<button class="vsure vbtn">${(o && o.otxt) ||
          _root.i18n.continue}</button>`;
        ...
        if (o && o.type) {
          const _ok = _mark.querySelector(".vsure");
    ...
    _root.loading.show();
    const query1 = new _root.v.Query("Comment");
    query1.equalTo("url", defaultComment.url);
    const query2 = new _root.v.Query("Comment");
  ...
  bind(option) {
    const _root = this;
    // Smile pictures
    const vsmiles = _root.el.querySelector(".vsmile-icons");
    Event.on("click", vsmiles, e => {
      const textField = _root.el.querySelector(".veditor");
    ...
    const comment_trigger = _root.el.querySelector(".comment_trigger");
    ...
    const parentQuery = (page_num = 1) => {
      _root.loading.show();
      const cq = _root.v.Query
        .doCloudQuery(`select nick, comment, link, rid, emailHash, isSpam
                                                   from Comment
                                                   where (rid='' or rid is not exists) and (url='${
                                                     defaultComment.url
                                                   }' or url='${`${defaultComment.url}/`}')
                                                   order by -createdAt
                                                   limit ${(page_num - 1) *
                                                     PAGE_SIZE},${PAGE_SIZE}`);
      cq.then(rets => {
        ...
        if (len) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            const _parent_vcard = insertComment(
              rets[i],
              _root.el.querySelector(".vlist"),
              false
            );
            _parent_vcard.setAttribute("style", "margin-bottom: .5em");
            nestQuery(_parent_vcard);
          }
          const _vpage = _root.el.querySelector(".vpage");
          _vpage.innerHTML =
            PAGE_SIZE * page_num < parent_count
              ? `<div id="vmore" class="more">${_root.i18n.more}</div>`
              : "";
          const _vmore = _vpage.querySelector("#vmore");
          if (_vmore) {
    ...
    const nestQuery = (vcard, level = 1) => {
      const _vchild = vcard.querySelector(".vcomment-children");
      const _vlist = _vchild.querySelector(".vlist");
      ...
      if (level >= MAX_NEST_LEVEL) {
        _root.v.Query.doCloudQuery(
          ...
          data => {
            const { count } = data;
            if (count > 0) {
              const _show_children_wrapper = _vchild.querySelector(
                ".vshow-children-wrapper"
              );
              _show_children_wrapper.setAttribute(
                "style",
                "display: block !important;"
              );
              _show_children_wrapper.innerHTML = `<span class="vshow-children" rid="${_id}">${_root.i18n.more}</span>`;
              const _show_children = _show_children_wrapper.querySelector(
                ".vshow-children"
              );
    ...
    const insertComment = (comment, vlist = null, top = true) => {
      ...
      const gravatar_url = `${GRAVATAR_BASE_URL +
        emailHash}?size=80&d=https%3a%2f%2fgravatar.loli.net%2favatar%2f9e63c80900d106cbbec5a9f4ea433a3e.jpg%3fsize%3d80`;
      // language=HTML
      _vcard.innerHTML = `<div class="vcomment-body">
        ...
        "nick"
      )}' class="vat" id="at-${comment.id}">${_root.i18n.reply}</a>
                                        <div class="vmeta-info">
                                            ${
                                              comment.get("link")
                                                ? `<a class="vname" href="${comment.get(
                                                    "link"
                                                  )}" target="_blank" rel="nofollow" > ${comment.get(
                                                    "nick"
                                                  )}</a>`
                                                : `<span class="vname">${comment.get(
                                                    "nick"
                                                  )}</span>`
                                            }
																						${
                                    ...
                                    <section class="text-wrapper"  id="comment-${
                                      comment.id
                                        ...
                                        <div class="vcomment">${comment.get(
                                          "comment"
                                        )}</div>
                                    </section>
                                </div>
      ...
      const _vlist = vlist || _root.el.querySelector(".vlist");
    ...
    for (const i in mapping) {
      if (mapping.hasOwnProperty(i)) {
        const _v = mapping[i];
        const _el = _root.el.querySelector(`.${i}`);
        inputs[_v] = _el;
    ...
    const getCache = () => {
      ...
      if (s) {
        ...
        if (s.mail != "") {
          const el = _root.el.querySelector(".visitor_avatar");
          el.setAttribute(
            "src",
            `${GRAVATAR_BASE_URL +
              crypto(
                s.mail.toLowerCase().trim()
    ...
    _root.reset = () => {
      ...
      _root.el
        ...
        .appendChild(_root.el.querySelector(".vinputs-wrap"));
    };
    ...
    const smile_btn = _root.el.querySelector(".vemoji-btn");
    const smile_icons = _root.el.querySelector(".vsmile-icons");
    ...
    const preview_btn = _root.el.querySelector(".vpreview-btn");
    const preview_text = _root.el.querySelector(".vpreview-text");
    ...
    const commitEvt = () => {
      ...
      // 声明类型
      const Ct = _root.v.Object.extend("Comment");
      // 新建对象
    ...
    const bindAtEvt = vcard => {
      ...
      Event.on("click", _vat, e => {
        ...
        // move inputs
        const _comment_el = vcard.querySelector(`#comment-${_id}`);
...
const getIp = function() {
  $.getJSON("https://api.ipify.org/?format=json", json => {
